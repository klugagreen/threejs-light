...
pointlight_1.position.set(0, -1500, 2000);
...
function modelLoader(scene, name, src, color,reCount) {
    ...
    loader.load(src, function (gltf) {
        ...
        // texture.offset.set((1 - 1 / tAspect) / 2, (1 - tAspect) / 2)        
        texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
        // texture.offset.set(0 , 0)
        texture.repeat.set(reCount, reCount);
        ...
        theModel.traverse((o) => {
            if (o.isMesh) {
                ...
                o.material = material
                if (textureSrc) o.material.map = texture
                noOfMeshes++
            }
        });
...
$.getJSON(config, function (data) {
    ...
    menuData.forEach(function (item, index) {
        modelLoader(scene, item.value, item.src, item.color,tAspect);
    })
});
...
// texture scroll
var t_clickflag = false;
var t_originX = -1;
var t_originMargin = -1;
$('#texture_controller_scroll').mousedown(function (event) {
    t_clickflag = true;
    t_originX = event.clientX;
    t_originMargin = parseInt($('#texture_controller_scroll').css("margin-left").split('px')[0]);
})

$('body').mousemove(function (event) {
    if (t_clickflag) {
        var tempX = t_originMargin + event.clientX - t_originX;
        var maxtemp = parseInt($('.texture_controller_bar').css("width").split('px')[0] - $('#texture_controller_scroll').css("width").split('px')[0]);
        if (tempX < 0) {
            tempX = 0
        } else if (tempX > maxtemp) {
            tempX = maxtemp
        }
        $('#texture_controller_scroll').css("margin-left", tempX);

        var scaleValue = 1 + tempX / 20;
        // camera.zoom = scaleValue;
        changeTextureRepeat(scaleValue);
        camera.updateProjectionMatrix();
    }
})
$('body').mouseup(function () { t_clickflag = false; })
$('#texture_controller_scroll').mouseup(function () { t_clickflag = false; })

const changeTextureRepeat = (reCount) => {
    tAspect = reCount;
    $.getJSON(config, function (data) {
        let menuData = [];
        menuData = data.menuData;
...
// light scroll
var l_clickflag = false;
var l_originX = -1;
var l_originMargin = -1;
$('#light_controller_scroll').mousedown(function (event) {
    l_clickflag = true;
    l_originX = event.clientX;
    l_originMargin = parseInt($('#light_controller_scroll').css("margin-left").split('px')[0]);
})

$('body').mousemove(function (event) {
    if (l_clickflag) {
        var tempX = l_originMargin + event.clientX - l_originX;
        var maxtemp = parseInt($('.light_controller_bar').css("width").split('px')[0] - $('#light_controller_scroll').css("width").split('px')[0]);
        if (tempX < 0) {
            tempX = 0
        } else if (tempX > maxtemp) {
            tempX = maxtemp
...
function onclick_col_sample(index) {
    ...
    if (setId != -1) {
        ...
        $.getJSON(config, function (data) {
            menuData = data.menuData;
            modelLoader(scene, menuData[setId].value, menuData[setId].src, arrTostr(selectModelColor),100);
        });
        ...
        $.getJSON(config, function (data) {
            ...
            function modelLoader_preview(scene, name, src, color) {
                ...
                loader.load(src, function (geometry) {
                    ...
                    texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
                    // texture.offset.set(0 , 0)
                    // texture.repeat.set(tAspect, tAspect);
...
function removeTexture() {
    ...
    $.getJSON(config, function (data) {
        let menuData = [];
        menuData = data.menuData;
        modelLoader(scene, menuData[setId].value, menuData[setId].src, arrTostr(selectModelColor),tAspect);
    })
}
